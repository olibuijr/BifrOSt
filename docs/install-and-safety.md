...
- **Wi-Fi:** the live system includes Linux's in-tree wireless drivers and Atheros, Broadcom, Intel, Marvell/NXP, MediaTek, and Realtek firmware; the installed system receives Arch's complete default firmware set. The packaged Broadcom STA driver is available in both. Adapters that require an out-of-tree, AUR, or vendor-only driver are not bundled. This broadens device coverage but is not a claim that every Wi-Fi adapter works.
